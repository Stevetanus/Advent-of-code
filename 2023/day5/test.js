...
const input = fs.readFileSync('crawler/src/day5/2023day5.txt', 'utf8');
...
for (const seed of data.seeds) {
  ...
  while (currentState !== endState) {
    const converter = data.converters.find(
      (converter) => converter.from === currentState
    );
    console.log(
      `[${seed}] val=${currentValue} from=${converter.from} to=${converter.to}`
    );

    for (const range of converter.ranges) {
      if (
        currentValue >= range.srcStart &&
        currentValue < range.srcStart + range.length
      ) {
        console.log(
          `[${seed}] val=${currentValue} from=${converter.from} to=${
            converter.to
          } in range ${range.srcStart} - ${range.srcStart + range.length}`
        );
